...
function SearchResult(props) {
  ...
  const getChatSearchApi = async () => {
    try {
      const callUrl =
        serverUrl +
        'chat/search?subjectName=' +
        (await selectedSubject.subject_name) +
        '&subjectProfessor=' +
        selectedSubject.subject_professor;
      ...
      for (const chatroom of chatSearchJson) {
        const callUrl =
          serverUrl + 'chat/mentor?chatNumber=' + chatroom.chat_number;
        const chatMentorResponse = await fetch(callUrl);
        const chatMentorJson = await chatMentorResponse.json();
        const searchedChatrom = {
          ...chatroom,
          mentor_name: chatMentorJson.user_name,
          mentor_recom: chatMentorJson.user_recom,
        };
  ...
  function menteeInMsg(chatroom) {
    ...
    for (const chat of chatrooms) {
      if (
        chatroom.subject_number.subject_name ===
        chat.subject_number.subject_name
      ) {
        alreadyIn = true;
        break;
      }
  ...
  function DisabledButton() {
    return (
      <TouchableOpacity
        style={styles.chatroomDisabledButton}
        onPress={() => disabledMsg()}>
  ...
  function ChatroomInButton(props) {
    if (props.chatroom.chat_mentee === props.chatroom.chat_max) {
      ...
      return <InButton chatroom={props.chatroom} />;
    }
  }
            ...
            <View style={styles.chatroomInfoWrap}>
              <View style={styles.chatroomMentorInfoWrap}>
                <View style={styles.mentorNameandRecomdWrap}>
                  <Text
                    style={{fontSize: 20, fontWeight: '500', color: '#3B2270'}}>
                    {chatroom.mentor_name}
                  </Text>
                  <Text
                    style={{fontSize: 12, fontWeight: '500', color: '#000000'}}>
                    {' '}
                    👍{chatroom.mentor_recom}
                  </Text>
                </View>
              ...
              <View style={styles.chatroomPartyInfoWrap}>
                <Text style={{fontSize: 15}}>
                  {chatroom.chat_mentee} / {chatroom.chat_max}
                </Text>
              </View>
            </View>
