...
function ChatList(props) {
  ...
  function menteeOut(chatroom) {
    Alert.alert('경고!', '멘티 신청을 취소하시겠습니까?', [
      {
        text: 'Cancel',
        style: 'cancel',
      },
  ...
  function MentorChatroomList() {
    if (mentorChatroomList.length === 0) {
      return (
        <Text style={{marginLeft: 49, fontSize: 14, marginTop: 10}}>
          등록된 맨토 채팅방이 없습니다.
        </Text>
      );
    } else {
          ...
          <View style={styles.mentorChatroomList}>
            {mentorChatroomList.map((chatroom, i) => (
              <View key={i} style={styles.mentorChatroom}>
                <Text
                  style={{marginLeft: '5%', fontSize: 20, color: '#3E3F42'}}>
    ...
    return (
      <View style={styles.chatroomListWrap}>
        <SearchBar
          ...
          MakeSubjectList={props.MakeSubjectList}
        />
