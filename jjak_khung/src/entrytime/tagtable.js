...
function TagTable(props) {
  ...
  const [appealTags, setAppealTags] = useState([
    [
      {tag: '#친절', pressed: false},
      {tag: '#웃음', pressed: false},
      {tag: '#상냥', pressed: false},
      {tag: '#유쾌', pressed: false},
    ],
    [
      {tag: '#차분', pressed: false},
      {tag: '#열정', pressed: false},
      {tag: '#개성적', pressed: false},
      {tag: '#귀여움', pressed: false},
    ],
    [
      {tag: '#듬직', pressed: false},
      {tag: '#감성적', pressed: false},
      {tag: '#외향', pressed: false},
      {tag: '#내향', pressed: false},
    ],
    [
      {tag: '#낙천', pressed: false},
      {tag: '#지적', pressed: false},
      {tag: '#성실', pressed: false},
      {tag: '#온화', pressed: false},
    ],
  ]);
  ...
  function UnpressedButton(props) {
    return (
      <TouchableOpacity
        style={styles.unpressedTagButton}
        onPress={() => handleTagButtonPressed(props.appealTag)}>
        <Text style={{fontSize: 17, color: '#000000'}}>
          {props.appealTag.tag}
        </Text>
  ...
  function TagButton(props) {
    if (props.appealTag.pressed) {
      return <PressedButton appealTag={props.appealTag} />;
    } else {
      return <UnpressedButton appealTag={props.appealTag} />;
    }
  }
