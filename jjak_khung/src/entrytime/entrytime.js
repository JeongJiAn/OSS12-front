...
function Entrytime(props) {
  ...
  const getChatUserApi = async () => {
    try {
      const callUrl =
        serverUrl + 'chat/user?userNumber=' + userInfo.user_number;
      ...
      for (const chatroom of chatrooms) {
        try {
          const callUrl =
            serverUrl + 'chat/room/list?chatNumber=' + chatroom.chat_number;
    ...
    if (subject !== '') {
      if (prof !== '') {
        for (const subj of subjectList) {
          if (
            subj.subject_name === subject &&
            subj.subject_professor === prof
          ) {
  ...
  return (
    <Stack.Navigator>
      <Stack.Screen name="chatlist" options={{headerShown: false}}>
        {props => (
          <ChatList
            ...
            chatrooms={chatrooms}
            mentorChatroomList={mentorChatroomList}
            menteeChatroomList={menteeChatroomList}
            userInfo={userInfo}
            serverUrl={serverUrl}
          />
        )}
      ...
      <Stack.Screen
        name="searchchatlistbyprof"
        options={{
          title: '"' + subject + '" ' + '교수님 검색',
          headerBackTitle: '뒤로',
          headerTintColor: '#ffffff',
          headerStyle: {backgroundColor: '#6667AB'},
        }}>
        {props => (
          <SearchChatListbyProf
            subjectList={subjectList}
            subject={subject}
            setProf={setProf}
          />
